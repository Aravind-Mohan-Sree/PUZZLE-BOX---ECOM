...
const home = async (req, res) => {
  try {
    ...
    const ratings = await reviewSchema.find(
      {},
      { productID: 1, averageRating: 1, createdAt: 1 }
    );
    ...
    const activeCategoryNames = Array.from(
      new Set(
        activeProducts.map((product) => product.productCategory.categoryName)
      )
    ...
    const topProducts = activeProducts
      ...
      .sort((a, b) => {
        const ratingA = ratingsMap[a._id.toString()] || {
          averageRating: 0,
          createdAt: new Date(0),
        };
        const ratingB = ratingsMap[b._id.toString()] || {
          averageRating: 0,
          createdAt: new Date(0),
        };
